...
-- Keep the table private from browser clients. Access is only through RPC functions below.
...
begin
  ...
  if not found or v_comment.password_hash <> crypt(v_password, v_comment.password_hash) then
    raise exception 'not_found_or_bad_password';
  end if;
